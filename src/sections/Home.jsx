...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const fullRoleText = baseText + currentRole;

    if (!isDeleting) {
      // Typing effect
      if (displayedText.length < fullRoleText.length) {
        timeout = setTimeout(() => {
  ...
  const renderAnimatedText = () => {
    ...
    // Vérifier si le rôle actuel est affiché
    if (displayedText.includes(currentRole)) {
      const beforeRole = displayedText.split(currentRole)[0];
      return (
        <>
          <span>{beforeRole}</span>
          <span className="text-[var(--primary-color)]">{currentRole}</span>
        </>
      );
    }

    // Si le texte est en cours de suppression ou au début
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-8 lg:gap-12 ">
          ...
          <div className="flex-1 max-w-xxl order-2 lg:order-1  ">
            <h1 className="text-2xl sm:text-3xl md:text-3xl lg:text-5xl font-bold mb-4 sm:mb-6 leading-tight text-center lg:text-left min-h-[120px] sm:min-h-[140px] md:min-h-[160px]">
              {renderAnimatedText()}
              {(displayedText.length < (baseText + roles[roleIndex]).length ||
                isDeleting) && <span className="ml-1 animate-pulse">|</span>}
            </h1>
